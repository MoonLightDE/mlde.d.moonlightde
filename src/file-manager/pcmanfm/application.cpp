...
void Application::init() {
    ...
    qApp->installTranslator(libFm_.translator());
...
void Application::onScreenResized(int num) {
    ...
    QRect rect = qApp->desktop()->screenGeometry(num);
    window->setGeometry(rect);
}

void Application::onWorkAreaResized(int num) {
    DesktopWindow* window = desktopWindows_.at(num);
    QRect rect = qApp->desktop()->availableGeometry(num);
    window->setWorkArea(rect);
}

DesktopWindow* Application::createDesktopWindow(int screenNum) {
    DesktopWindow* window = new DesktopWindow();
    QRect rect = qApp->desktop()->screenGeometry(screenNum);
    window->setGeometry(rect);
    rect = qApp->desktop()->availableGeometry(screenNum);
    window->setWorkArea(rect);
...
Application * Application::instance() {
    if (!m_application) {
        int ac = qApp->arguments().size();
        char ** argv = (char **) malloc(sizeof ( char *) * ac);
        for (int i = 0; i < ac; i++) {
            argv[i] = (char * ) malloc(sizeof(qApp->arguments().at(i).toLocal8Bit()));
            strcpy(argv[i], qApp->arguments().at(i).toLocal8Bit());
        }
        //        strs[i] = malloc(strlen("foo") + 1);
        //        strcpy(strs[i], "foo");
        //        char * * const av = 
        m_application = new Application(ac, argv);
    }
