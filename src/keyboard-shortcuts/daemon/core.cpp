...
InnerAction* Core::addInnerAction(QPair<QString, qulonglong> &result, const QString &shortcut, const QString &description) {
    log(LOG_INFO, "addInnerAction shortcut:'%s' description:'%s'", qPrintable(shortcut), qPrintable(description));
    ...
    mIdsByShortcut[newShortcut].insert(id);
    InnerAction* innerAction = new InnerAction(this, shortcut, description);
    mShortcutAndActionById[id] = qMakePair<QString, InnerAction *>(newShortcut, innerAction);

    log(LOG_INFO, "addInnerAction shortcut:'%s' id:%llu", qPrintable(newShortcut), id);
...
void Core::changeInnerActionShortCut(qulonglong &result, InnerAction &action) {
}
