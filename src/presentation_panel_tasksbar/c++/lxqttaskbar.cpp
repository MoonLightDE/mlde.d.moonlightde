...
void LxQtTaskBar::settingsChanged() {
    // Using MoonLightDE core settings, old settings reamains in coments.
    ...
    mButtonWidth = settings->value("buttonWidth", 400).toInt();
...
//    mCloseOnMiddleClick = mPlugin->settings()->value("closeOnMiddleClick", true).toBool();
    mCloseOnMiddleClick = settings->value("closeOnMiddleClick", true).toBool();
...
void LxQtTaskBar::realign() {
    ...
    if (mButtonStyle == Qt::ToolButtonIconOnly) {
        // Horizontal + Icons **************
        mLayout->setRowCount(1 /*panel->lineCount()*/);
